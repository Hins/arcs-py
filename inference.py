...
def main(input_path, result_path):
    ...
    _, gold = fileio.read_conll_deps("./sample-data/dep_train_result.txt", True)
    gold = filter_non_projective(gold)
    model = Classifier({}, [0, 1, 2, 3])

    parser = Parser(model, feature_extractor)
    print('performing %d iterations' % 10)
    for i in range(0, 10):
        correct_iter = 0
        all_iter = 0
        random.shuffle(gold)
    ...
    for file in Path(input_path).glob('**/*.txt'):
        ...
        sentences, test = fileio.read_conll_deps(str(file), False)
        test = filter_non_projective(test)
        size = len(test)

        test_start_time = time.time()
        for idx, gold_test_sent in enumerate(test):
            arcs = set(parser.run(gold_test_sent))
            arc_dict = {}
            for arc in arcs:
                arc_dict[arc[1]] = arc[0]
            sentence_json_obj = {}
            sentence_json_obj["ID"] = idx
            sentence_json_obj["text"] = "".join(sentences[idx])
            sentence_json_obj["words"] = []
            for id, token in enumerate(arcs):
                if id == len(sentences[i]):
                    break
                token_obj = {}
                token_obj["id"] = id + 1
                token_obj["form"] = sentences[idx][id]
                token_obj["head"] = arc_dict[id]
                token_obj["pos"] = ""
                token_obj["deprel"] = ""
    ...
    return size, test_time, json_obj